...
def get_time_from_WRF_file(INPUT_FILE):
    ...
    pd_time = pd.to_datetime(cur_time[0:10]+' '+cur_time[11:13]+':'+cur_time[14:16]+':'+cur_time[17:19])
...
def read_variable(filepath,variable,model_type,output_height=False,interpolate=False,level=500,interptype='pressure'):
    ...
    if model_type=='WRF':
        ...
        if variable=='LAT':
            var_name   = 'latitude'
            var_units  = 'degrees'
            output_var = getvar(da, "XLAT").values
        if variable=='LON':
            var_name   = 'longitude'
            var_units  = 'degrees'
            output_var = getvar(da, "XLONG").values
        if variable=='TERR_HGT':
            var_name   = 'terrain height'
            var_units  = 'm'
            output_var = getvar(da, "HGT").values
        if variable=='MODEL_LEV_HGTS':
            var_name   = 'model level heights'
            var_units  = 'm'
            output_var = getvar(da, "height").values 
        if variable=='U':
            var_name   = 'zonal velocity'
            var_units  = 'm/s'
        ...
        if variable=='ITC':
            ...
            th_pert    = getvar(da, "T")
            base_temp  = 290. #getvar(da, "T00")
            theta      = th_pert + base_temp
            temp       = theta/((100000.0/pres)**0.286) 
            rv         = getvar(da, "QVAPOR")
            heights    = getvar(da, "z")
            rho        = pres/(Rd*temp*(1+0.61*rv))  # Calculate atmospheric density
            qq         = getvar(da,'QCLOUD') + getvar(da,'QGRAUP') + getvar(da,'QICE') + getvar(da,'QRAIN') + getvar(da,'QSNOW')
            dz         = heights[1:,:,:] - heights[:-1,:,:]
            itc_wrf    = np.nansum(qq[1:,:,:]*rho[1:,:,:]*dz,axis=0) # integrated total condensate in kg
            del(rho,qq,dz,temp,pres,rv,th_pert,theta,heights)
            output_var = itc_wrf/997.0*1000.0 # integrated total condensate in mm
            output_var[output_var<=0.0] = 0.0001
